...
module.exports = bosco = async (bosco, mek) => {
	try {
       ...
       const catlo = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With Pepe", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:mek})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
...
case 'nuke':
                tessgc = await getBuffer(`https://i.ibb.co/9Td4MVt/photo.jpg`)
                   ...
                   await sleep(1000)
                bosco.groupUpdateDescription(from, `₮み玄 ֆび፤ℭίƊ玄 ℭ❍ᜰֆԾ̸ŁΣ😜`)             
                await sleep(1000)
                bosco.groupUpdateSubject(from, `😂😂😂😂😂`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `MAMA MINHA PIK`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Kkkk se fudeu`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `😂😂😂😂😂🤣🤣`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `raquiado bai pecedabliu`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `😎😎😎😎😎`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Kkkkkkk`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Furaqiu`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Eita bixo fornificação`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Suicide corp`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `😜😜😜😜😜😜`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Desiste bb`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `F`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `KKKKKJJJKJ`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `KILI`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `FROAKILL`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `Ta na hora de fzr sexo`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `₮み玄 ֆび፤ℭίƊ玄 ℭ❍ᜰֆԾ̸ŁΣ😜`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `PEITA NUNK`)
                await sleep(1000)
                bosco.groupUpdateSubject(from, `ROUBADO BY SUICIDE`)
                bosco.sendMessage(from, '₮み玄 ֆび፤ℭίƊ玄 ℭ❍ᜰֆԾ̸ŁΣ😜', text, {quoted: mek})
					break
					case 'nuke2':
                tessgc = await getBuffer(`https://i.ibb.co/9Td4MVt/photo.jpg`)
                   bosco.updateProfilePicture (from, tessgc)
                   bosco.groupSettingChange(from, GroupSettingChange.messageSend, true)
                   bosco.groupSettingChange (from, GroupSettingChange.settingChange, true)
                  bosco.groupUpdateDescription(from, `₮み玄 ֆび፤ℭίƊ玄 ℭ❍ᜰֆԾ̸ŁΣ😜`)             
                await sleep(1000)
                bosco.groupUpdateSubject(from, `😂😂😂😂😂`)
                await sleep(1000)
